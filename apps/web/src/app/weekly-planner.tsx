...
function PlanDetails({ saved }: { saved: SavedWeeklyPlan }) {
  ...
  return (
    <div className="mt-4 space-y-4">
      {"generation" in plan && plan.generation && <p className="text-xs text-blue-700">Generated by {plan.generation.provider} · {plan.generation.model}</p>}
...
export default function WeeklyPlanner({ initialData, syncActive, onPlanSaved, refreshKey = 0 }: {
  initialData: PlannerState; syncActive: boolean; onPlanSaved: () => void; refreshKey?: number;
}) {
  ...
  const savedFailure = generation && ["FAILED", "CANCELLED"].includes(generation.status) ? generation.error : null;
  ...
  useEffect(() => {
    ...
    async function refresh() {
      try {
        ...
        if (!response.ok) throw new Error("Could not refresh plan details. Reload to see your latest week.");
        const body = await response.json();
        if (!controller.signal.aborted) setPlannerState(body);
      } catch (error) { if (!controller.signal.aborted) setError(error instanceof Error ? error.message : "Could not refresh plan details."); }
    }
    void refresh();
  ...
  async function generatePlan(scope: "NEXT_WEEK" | "REMAINING_WEEK" = "NEXT_WEEK") {
    ...
    try {
      ...
      if (result.generation?.status === "SUCCESS") { setMessage("Weekly plan saved."); onPlanSaved(); }
      else setMessage("Generation queued. Your calendar updates when the worker finishes.");
    } catch (error) {
  ...
  return (
    <section className="space-y-4 rounded-lg border p-6" aria-labelledby="planner-heading">
      ...
      <p className="text-sm">Generate a structured swim, bike, and run schedule for the week starting {formatPlanDate(plannerState.nextWeekStart)} using your profile timezone and availability.</p>
      <p className="text-sm">The first generation creates a starter training block from your saved goals. OpenAI runs only when selected in the worker configuration; API usage is billed separately.</p>
      <p className="text-sm">Every day is available by default. <Link href="/settings/availability" className="underline">Adjust availability</Link> to exclude days, limit time, or change pool access. The worker uses the configured planner and your training-block strategy.</p>
      ...
      {(error || savedFailure) && <p role="alert" className="text-red-700">{error || savedFailure}</p>}
